...
public class LinkServiceImpl implements LinkService {
    ...
    @Override
    public List<Link> getLinksByCommodityId(String commodityId) {
	...
	buyNodes.stream().filter(buy -> (buy.getPrice().compareTo(BigDecimal.ZERO) > 0)).forEach(buy -> sellNodes
		.stream().filter(sell -> (sell.getPrice().compareTo(BigDecimal.ZERO) > 0)).forEach(sell -> {
	...
	commodities.stream().forEach(commodity -> {
	    links.addAll(getLinksByCommodityId(commodity.getId()));
	});
	...
	if (buyPrice.compareTo(BigDecimal.ZERO) != 0) {
	    ...
	    BigDecimal profitMargin = profit.divide(buyPrice, BigDecimal.ROUND_HALF_UP)
